...
export function CalendarBodyTableTbody(props) {
    ...
    const daysInWeek = 7;
    const weeks = Array.from({length: Math.ceil(dates.length / daysInWeek)}, (_, i) => dates.slice(i * daysInWeek, i * daysInWeek + daysInWeek));
            ...
            {weeks.map((week, i) => (
                <tr key={i}>
                    {week.map(({ date, isInCurrentMonth, isCurrentDay}) => (
                        <td
                            className={`${isInCurrentMonth ? '' : 'ui-datepicker-other-month'} ${isCurrentDay ? 'ui-datepicker-today' : ''}`}
                            key={date.toString()}
                        >
                        {date.format('D')}
                        </td>
